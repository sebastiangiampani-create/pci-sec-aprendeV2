(() => {
  ...
  const valueMetric=r=>Number(r?.total)?`${Number(r.used||0)}/${Number(r.total)} · ${pct(r)}%`:pct(r)==null?'—':String(pct(r))+'%';
  ...
  function metricCard(label,result,detail=''){
    const value=pct(result);
    return `<article class="v93d-metric"><span>${esc(label)}</span><strong>${valueMetric(result)}</strong><small>${esc(detail)}</small>${value==null?'':`<div class="v93d-bar"><i style="width:${Math.min(100,value)}%"></i></div>`}</article>`;
  }
  ...
  function componentRows(result){
    ...
    if(!rows.length)return'';
    return `<div class="v93d-components">${rows.map(x=>`<section><header><div><small>${esc(x.kind||'Componente')}</small><strong>${esc(x.label)}</strong></div><b>${x.used}/${x.total} · ${x.percent}%</b></header>${(x.axes||[]).length?`<div class="v93d-axes">${x.axes.map(a=>`<div><span>${esc(a.axis)}</span><strong>${a.used}/${a.total} · ${a.percent}%</strong></div>`).join('')}</div>`:''}</section>`).join('')}</div>`;
  }
  ...
  function formatRows(rep,levelGroups,allGroups){
    ...
    return types.map(type=>{
      ...
      return `<section class="v93d-format">
        <div class="v93d-format-head"><div><span>${esc(typeLabel(type))}</span><small>${spaces.length} espacio${spaces.length===1?'':'s'}</small></div><strong>${valueMetric(result)}</strong></div>
        <div class="v93d-space-list">${spaces.map(g=>{
          const r=coverage()?.formatCoverage?.(g)||{};
          return `<div><span>${esc(groupName(g))}</span><strong>${valueMetric(r)}</strong></div>`;
        }).join('')}</div>
      </section>`;
    }).join('');
  }
  ...
  function levelCard(area,year,areaGroups,allGroups){
    ...
    return `<article class="v93d-level">
      <header class="v93d-level-head"><div><span>Nivel ${year}</span><h4>${esc(area)}</h4><p>${esc(basis)}</p></div><strong>${valueMetric(result)}</strong></header>
  ...
  function areaCard(area,allGroups){
    ...
    return `<section class="v93d-area">
      <header class="v93d-area-head"><div><span>Agrupamiento · trayectoria</span><h3>${esc(area)}</h3><p>Unión de contenidos utilizados en toda la trayectoria disponible.</p></div><strong>${valueMetric(trajectory)}</strong></header>
